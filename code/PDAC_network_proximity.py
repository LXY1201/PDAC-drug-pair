...
if __name__ == "__main__":
    ...
    FILE1 = "data/FDA_drug_id.txt"
    FILE2 = "data/PDAC_driver.txt"
    OUTPUT_DIR = "result/"  
    REPEAT = 1000
    MEASURE = "CLOSEST"
    
    Net = Network("data/adj/HumanInteractome.adj", "data/adj/HumanInteractome.npy")
    Net.PrepareBins(100)
    ...
    with open(FILE2, "r") as f2:
        lines = f2.readlines()
        for line in tqdm(lines, desc="Processing genes"):
            gene_id = line.strip().split()[1]  
            MAPPING2 = ToDictLine(FILE2, line, Net) 
    

            output_file = f"{OUTPUT_DIR}PDAC_drug_{gene_id}.txt"
            ...
            for k1 in sorted(MAPPING1.keys()):
                for k2 in sorted(MAPPING2.keys()):
                    ...
                    z, p = Z_Score(d, b)
                    results.append((k1, k2, d, z, p))
    

            results.sort(key=lambda x: x[2])
            with open(output_file, "w") as fo:
                fo.write("Drug\tdisease\td\tz\tp\n")
